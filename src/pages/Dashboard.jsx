...
import {
  ...
  ShieldCheck, Crown, Heart, Home, Edit
} from "lucide-react";
...
const Dashboard = () => {
  ...
  // Hybrid User Object: Pehle Redux dekho, nahi toh Clerk se map karo
  ...
  // Role Logic (Keeping your logic 100% same)
  ...
  // Sync Clerk with MongoDB/Redux to get old token & reports
  useEffect(() => {
  ...
  const loadReports = async () => {
    try {
      ...
      setReports(data);
    } catch (e) { toast.error("Failed to load activity feed"); }
  };
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      const { data } = await api.post("/api/ai/analyze-image", {
        image: photoBase64, description: form.description,
      });
  ...
  const selectStyle = {
    padding: "0.9rem", borderRadius: "12px", background: "var(--bg-card)", color: "var(--text-main)", border: "1px solid var(--border)", width: "100%",
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-main)", color: "var(--text-main)", fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: `2.5px solid ${badge.color}`, borderRadius: "32px", padding: "3rem", marginBottom: "3.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: `0 20px 50px ${badge.color}15` }}>
          <div>
            ...
            <p style={{ fontSize: "1.2rem", opacity: 0.8, marginTop: "10px" }}>
              Welcome back <b style={{ color: badge.color }}>{getRoleLabel(user?.role)}</b> {user?.name}. You rescued <b>{rescuedCount}</b> animals. 🇮🇳
            </p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: "2rem" }}>
          {reports.length === 0 ? (
            <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "4rem", opacity: 0.5 }}>
              ...
              <p style={{ fontSize: "1.2rem", fontWeight: 600 }}>No reports yet. Be the first!</p>
            </div>
          ) : (
            reports.map((report) => (
              <div key={report._id} style={{ background: "var(--bg-card)", borderRadius: "28px", overflow: "hidden", border: "1px solid var(--border)", position: "relative" }}>
                ...
                <div style={{ padding: "1.5rem" }}>
                  <h3 style={{ fontWeight: 800, fontSize: "1.2rem", display: "flex", alignItems: "center", gap: "8px" }}>{SPECIES_EMOJIS[report.animalType] || "🐾"} {report.animalType?.toUpperCase()} Alert</h3>
                  <p style={{ fontSize: "0.9rem", opacity: 0.7, display: "flex", alignItems: "center", gap: "4px" }}><MapPin size={14} /> {report.location}</p>
                  
                  {report.aiAdvice && (
                    <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid #22c55e30", borderRadius: "12px", padding: "0.7rem", marginBottom: "12px" }}>
                      <p style={{ fontSize: "0.75rem", color: "#16a34a", fontWeight: 700, margin: "0 0 4px" }}>🤖 AI Advice</p>
                      <p style={{ fontSize: "0.78rem", margin: 0, opacity: 0.8 }}>{report.aiAdvice}</p>
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "12px" }}>
                    ...
                    {report.status === "pending" && isGov && <button onClick={() => updateStatus(report._id, "rescued")} style={{ flex: 1, background: "#22c55e", color: "#fff", borderRadius: "12px", fontWeight: 900 }}>🚑 Rescue</button>}
                    {report.status === "rescued" && isNGO && <button onClick={() => updateStatus(report._id, "sheltered")} style={{ flex: 1, background: "#8b5cf6", color: "#fff", borderRadius: "12px", fontWeight: 900 }}>🏠 Shelter</button>}
                  </div>
                </div>
              </div>
            ))
      ...
      {/* FAB - ADD EMERGENCY */}
      <button onClick={() => setShowModal(true)} style={{ position: "fixed", bottom: "40px", right: "40px", background: "#f59e0b", color: "white", width: "75px", height: "75px", borderRadius: "50%", zIndex: 1000, boxShadow: "0 10px 25px rgba(245,158,11,0.5)", border: "none", cursor: "pointer" }}>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 5000, backdropFilter: "blur(15px)", padding: "20px" }}>
          <div style={{ background: "var(--bg-card)", borderRadius: "32px", padding: "40px", width: "100%", maxWidth: "550px", maxHeight: "95vh", overflowY: "auto", border: "1px solid var(--border)" }}>
            ...
            <div onClick={() => fileInputRef.current.click()} style={{ height: "200px", border: "2px dashed var(--border)", borderRadius: "20px", display: "flex", justifyContent: "center", alignItems: "center", cursor: "pointer", background: "rgba(255,255,255,0.03)", marginBottom: "20px", overflow: "hidden" }}>
              {photoPreview ? <img src={photoPreview} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ textAlign: "center", opacity: 0.5 }}><Camera size={40} /><p>Take Animal Photo 📸</p></div>}
            </div>
            <input type="file" ref={fileInputRef} hidden onChange={(e) => { const file = e.target.files[0]; if (file) { setPhotoPreview(URL.createObjectURL(file)); const reader = new FileReader(); reader.readAsDataURL(file); reader.onloadend = () => setPhotoBase64(reader.result); }}} accept="image/*" />

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
              <button onClick={getLiveLocation} style={{ background: "#3b82f6", color: "white", padding: "12px", borderRadius: "50px", border: "none", fontWeight: "bold" }}>{isLocationLoading ? <Loader2 className="animate-spin" /> : "📍 GPS Lock"}</button>
              <button onClick={handleAiAnalysis} disabled={isAiLoading || !photoBase64} style={{ background: "#10b981", color: "white", padding: "12px", borderRadius: "50px", border: "none", fontWeight: "bold" }}>{isAiLoading ? <Loader2 className="animate-spin" /> : "✨ AI Scan"}</button>
            </div>

            <form onSubmit={handleSubmit}>
              <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} style={{ width: "100%", padding: "1rem", borderRadius: "15px", marginBottom: "12px", background: "var(--bg-main)", color: "var(--text-main)", border: "1px solid var(--border)" }} placeholder="📍 Landmark / Location" />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                <select value={form.animalType} onChange={(e) => setForm({ ...form, animalType: e.target.value })} style={selectStyle}>
                  {Object.keys(SPECIES_EMOJIS).map((s) => <option key={s} value={s}>{SPECIES_EMOJIS[s]} {s.toUpperCase()}</option>)}
                </select>
                <select value={form.urgency} onChange={(e) => setForm({ ...form, urgency: e.target.value })} style={selectStyle}>
                  <option value="low">🟢 Low</option><option value="medium">🟡 Medium</option><option value="high">🔴 High</option>
                </select>
              </div>
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} style={{ width: "100%", padding: "1rem", borderRadius: "15px", marginBottom: "20px", background: "var(--bg-main)", color: "var(--text-main)", border: "1px solid var(--border)", minHeight: "80px" }} placeholder="Condition details..." />
              <button type="submit" style={{ width: "100%", background: "#f59e0b", color: "white", padding: "1rem", borderRadius: "50px", border: "none", fontWeight: "black" }}>{editingReport ? "Update Report ✅" : "Submit Alert 🐾"}</button>
            </form>
          </div>
        </div>
      )}
      ...
      <div ref={certificateRef} style={{ position: "absolute", left: "-9999px", top: 0, width: "1123px", height: "794px", background: "white", overflow: "hidden" }}>
        ...
        <div style={{ textAlign: "center", padding: "80px" }}>
          <img src="/ashok.png" style={{ width: "65px" }} alt="Emblem" />
          <h1 style={{ fontSize: "80px", margin: 0 }}>CERTIFICATE</h1>
          <h2 style={{ fontSize: "50px", color: "#D4762A" }}>{user?.name}</h2>
          <p style={{ fontSize: "20px" }}>For successfully rescuing <b>{rescuedCount} stray animals</b> via <b>PawAlert</b>.</p>
